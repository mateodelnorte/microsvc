...
module.exports.addAllCommitAndPush = (message, options, cb) => {
  ...
  return add('.', { cwd: options.cwd },
    commit.bind(commit, message, { cwd: options.cwd },
      push.bind(push, 'master', { cwd: options.cwd },
        cb)));
}
...
module.exports.init = (args, options, cb) => {
  ...
  cp.exec(command, { cwd: path.resolve(process.cwd(), args) }, (err, stdout, stderr) => {
    if (err || stderr.toString().length) return cb(err);
    console.log(stdout);
    cb();
  });
...
var remoteAddOrigin = module.exports.remoteAddOrigin = function (owner, name, options, cb) {
  if (typeof options === 'function') {
    cb = options;
    options = {};
  }
